...
| **Hugging Face Transformers (Node.js)** | The JavaScript/Node.js implementation for running transformer models locally. `@huggingface/transformers` for embedders and rerankers; on desktop platforms it relies on `onnxruntime-node` for native inference. |
